...
class BatteryModel {
  ...
  getBatteryStateDisplayValue(BatteryState batteryStateVal) { 
    final stateVal = batteryStateVal.toString().substring(batteryStateVal.toString().indexOf('.')+1);
  ...
  BatteryModel fromJson(Map<String, dynamic> json) {
      ...
      currBatteryLevel = int.tryParse(json['currBatteryLevel']) ?? currBatteryLevel;
      currBatteryDateTime = DateTime.tryParse(json['currBatteryDateTime']) ?? currBatteryDateTime; 
      notifyBatteryState = getBatteryStatefromString(json['notifyBatteryState']) ?? notifyBatteryState; 
      notifyBatteryLevel = int.tryParse(json['notifyBatteryLevel']) ?? notifyBatteryLevel;
      backgroundColor = Color(int.parse(json['backgroundColor'])) ?? backgroundColor;
