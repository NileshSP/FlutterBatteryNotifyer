...
class SplashScreenState extends State<SplashScreen> {
  ...
  StreamController<double> periodicController;
  Stream<double> periodicStream;
  ...
  void initState() {
    ...
    periodicStream = periodicController.stream.asyncMap((pVal) async { return pVal; });
    periodicSubscription = Stream.periodic(Duration(seconds: 1),(e) { return ((e+1)*.20); }).listen((e) { periodicController.add(e); });
  }

  Future loadData() async {
    bMod = BatteryModel(appTitle: widget.title);
    await bMod.getPersistedValues();
    batteryDetails = BatteryDetails(batteryMod: bMod);
    const timerDuration = Duration(milliseconds: 500);
    Timer(timerDuration, () => Navigator.of(context).pushReplacement(
      //MaterialPageRoute(builder: (BuildContext context) => 
        PageTransition(widget: batteryDetails, tranType: 'animatedopacity', tranDuration: 600)
                            ...
                            CircleAvatar(
                              ...
                              child: Container(
                                ...
                                image: DecorationImage(
                                  colorFilter: ColorFilter.mode(bgColor, BlendMode.overlay),
                                  image: AssetImage("images/batteryalert.png"),
                                  fit: BoxFit.cover,
                                ),
                              ))
                            ),
